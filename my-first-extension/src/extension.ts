...
// STATE
// ─────────────────────────────────────────────
let _currentModelCommand: string | null = null;
let _requestedModelsList: boolean = false;
let _currentModelsData: string[] | null = null;
// Callbacks chờ /models trả về
let _modelsWaiters: Array<(models: string[]) => void> = [];
...
async function injectTokenNewFormat(
  dbPath: string,
  accessToken: string,
  refreshToken: string,
  expiry: number,
  ...
  try {
    ...
    try {
      if (process.platform === "linux") {
        execSync(
          `pkexec bash -c "cp '${tmpPath}' '${filePath}' && chmod 644 '${filePath}'"`,
          { timeout: 30000 },
        );
      ...
      throw new Error(
        `Permission denied writing to ${filePath}. Run VS Code as Administrator.`,
      );
    }
    try {
      fs.unlinkSync(tmpPath);
...
function isScriptInjected(wbPath: string): boolean {
  try {
...
function installScript(context: vscode.ExtensionContext): boolean {
  ...
  const scriptPathTemplate = path.join(
  ...
  try {
    ...
    html = html.replace(
      new RegExp(`${TAG_START}[\\s\\S]*?${TAG_END}`, "g"),
      "",
    );
    ...
    vscode.window.showErrorMessage(
      `[AG] Need Administrator rights to inject script!`,
    );
    return false;
  }
}
...
function writeDeviceProfile(storagePath: string, profile: DeviceProfile): void {
  ...
  syncServiceMachineId(
    path.join(path.dirname(storagePath), "state.vscdb"),
    profile.devDeviceId,
  );
}

async function syncServiceMachineId(
  dbPath: string,
  serviceId: string,
): Promise<void> {
...
function startServer(context: vscode.ExtensionContext) {
  ...
  server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && url === "/switch") {
      try {
        const payload: SwitchPayload = JSON.parse(await readBody(req));
        ...
        if (result.ok) {
          ...
          vscode.window.showInformationMessage(
            `AG Switch: Switching to ${payload.email}...`,
          );
          setTimeout(
            () =>
    ...
    if (req.method === "POST" && url === "/model") {
      try {
        ...
        if (!payload.model) return json(400, { error: "Missing field: model" });
